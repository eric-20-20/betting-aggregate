"""
BettingPros MLB prop-bets board ingestion script.

Targets:
  https://www.bettingpros.com/mlb/picks/prop-bets/

Output:
  - out/raw_bettingpros_prop_bets_mlb.json
...
PROPS_PAGE_URL = "https://www.bettingpros.com/mlb/picks/prop-bets/"
DEFAULT_MIN_PROJ_DIFF = 0.5
USER_AGENT = (
...
def _capture_initial_payloads(page, date: str) -> Tuple[Dict[str, Any], Dict[str, Any], str]:
    ...
    def on_response(resp):
        url = resp.url
        try:
            if captured["events"] is None and url.startswith("https://api.bettingpros.com/v3/events?sport=MLB"):
                captured["events"] = resp.json()
            elif captured["markets"] is None and url.startswith("https://api.bettingpros.com/v3/markets?sport=MLB&market_category=player-props"):
    ...
    if not isinstance(captured.get("events"), dict):
        raise RuntimeError("Failed to capture BettingPros MLB events payload")
    if not isinstance(captured.get("markets"), dict):
        raise RuntimeError("Failed to capture BettingPros MLB markets payload")
    if not captured.get("props_url"):
        raise RuntimeError("Failed to capture BettingPros MLB props API URL")
...
def _extract_event_map(events_payload: Dict[str, Any], target_date: str) -> Dict[int, Dict[str, Any]]:
    ...
    for event in events_payload.get("events") or []:
        ...
        if event_id is None:
            continue
        if _scheduled_game_day(event.get("scheduled")) != target_date:
            continue
...
def _extract_records(
    ...
    for prop in props:
        ...
        raw_fp = sha256_digest(
            f"bettingpros_props|MLB|{event_id}|{player_name}|{stat_key}|{recommended_side}|{line}|{target_date}"
        )
    ...
    if debug:
        print(f"[bettingpros_props_mlb] extracted={len(records)} skipped={skipped}")

    return records, {"skipped": skipped, "side_counts": side_counts, "stat_counts": stat_counts}


def ingest_bettingpros_prop_bets_mlb(
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Ingest BettingPros MLB prop-bets board")
    ...
    records, dbg = ingest_bettingpros_prop_bets_mlb(
    ...
    print(
        f"[bettingpros_props_mlb] date={dbg.get('date')} total={dbg.get('total')} "
    ...
    if args.dry_run:
        print("[bettingpros_props_mlb] Dry run — not writing output files")
        return

    ensure_out_dir()
    raw_path = os.path.join(OUT_DIR, "raw_bettingpros_prop_bets_mlb.json")
    write_records(raw_path, records)
    print(f"[bettingpros_props_mlb] Wrote raw → {raw_path}")

    from src.normalizer_bettingpros_props_mlb import normalize_bettingpros_props_records
    ...
    norm_path = os.path.join(OUT_DIR, "normalized_bettingpros_prop_bets_mlb.json")
    write_json(norm_path, normalized)
    eligible = sum(1 for r in normalized if r.get("eligible_for_consensus"))
    print(f"[bettingpros_props_mlb] Wrote normalized → {norm_path} ({eligible}/{len(normalized)} eligible)")
