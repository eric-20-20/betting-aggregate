...
from src.normalizer_bettingpros_nba import _build_event_keys, _parse_dt

ALLOWED_MARKETS = {"player_prop"}
# Canonical team codes — aligned with data_mlb.py (DataStore)
MLB_TEAM_CODES = {
    "ARI", "ATL", "BAL", "BOS", "CHC", "CHW", "CIN", "CLE", "COL", "DET",
    "HOU", "KCR", "LAA", "LAD", "MIA", "MIL", "MIN", "NYM", "NYY", "OAK",
    "PHI", "PIT", "SDP", "SEA", "SFG", "STL", "TBR", "TEX", "TOR", "WSN",
}
# Map short/alternate codes to canonical codes (BettingPros API uses short forms)
_SHORT_CODE_MAP = {
    "CWS": "CHW",
    "KC": "KCR",
    "SD": "SDP",
...
def _map_mlb_team(alias: Optional[str]) -> Optional[str]:
    if not alias:
        return None
    raw = str(alias).strip()
    if not raw:
        return None
    upper = raw.upper()
    # Map short codes (CWS→CHW, SF→SFG, etc.) before checking canonical set
    if upper in _SHORT_CODE_MAP:
        return _SHORT_CODE_MAP[upper]
    if upper in MLB_TEAM_CODES:
        return upper
    norm = re.sub(r"\s+", " ", raw.replace(".", " ")).strip().lower()
    return MLB_TEAM_ALIASES.get(norm)


def normalize_bettingpros_props_record(
    raw: Dict[str, Any],
    sport: str = "MLB",
    min_proj_diff: float = 0.5,
) -> Dict[str, Any]:
    ...
    away_team = _map_mlb_team(raw.get("away_team"))
    home_team = _map_mlb_team(raw.get("home_team"))
...
def normalize_bettingpros_props_records(
    ...
    if debug:
        ...
        print(
            f"[DEBUG] BettingPros Props MLB normalized: total={total} eligible={elig} "
            f"ineligible={inelig_total} removed_dupes={removed}"
        )
        if inelig_reasons:
            print(f"[DEBUG] BettingPros Props MLB ineligibility: {inelig_reasons}")
